...
\item \hl{How to generate GBM stock path ?}
\begin{itemize}
	\item generate path for $\frac{dS_t}{S_t}=\mu dt + \sigma dW_t$
	\item discretise $[0..T]$ into $m$ steps of size $\delta t = \frac{T}{m}$
	\item let $t_j=j\delta t $
	\item $S_{t_{j+1}} - S_{t_{j}} = \int_{t_j}^{t_{j+1}} \mu S_t dt + \int_{t_j}^{t_{j+1}} \sigma S_t dW_t dt  $
	\item \hly{Method 1}: Simple method (but can end up with negative paths)
	\begin{itemize}
		\item  $\int_{t_j}^{t_{j+1}} \mu S_t dt \approx \mu S_{t_j} (t_{j+1} - t_{j}) = \mu S_{t_j} \delta t $
		\item  $\int_{t_j}^{t_{j+1}} \sigma S_t dW_t dt \\ \approx \sigma S_{t_j} (W_{t_{j+1}} - W_{t_{j}}) \\ \approx \sigma S_{t_j} \sqrt {t_{j+1} - t_{j}} N(0,1)
		\\ = \sigma S_{t_j} \sqrt {\delta t} N(0,1)$
		\item $S_{t_{j+1}} - S_{t_{j}} = \mu S_{t_j} (t_{j+1} - t_{j}) + \sigma S_{t_j} \sqrt {\delta t} N(0,1)$
		\item $S_{t_{j+1}}  = S_{t_{j}} \myblp 1+\mu \delta t + \sigma \sqrt {\delta t} N(0,1) \mybrp $
		
	\end{itemize}
	\item \hly{Method 2}: with Ito - No negative paths
	\begin{itemize}
	\item  $\frac{dS_t}{S_t}=\mu dt + \sigma dW_t$
	\item  $d (\ln (S_t)) \myeq{Ito} (\mu - \frac{\sigma^2}{2}) dt + \sigma dW_t$ \hlr{\kreuz}
	\item  $ \ln(S_{t_{j+1}}) - \ln(S_{t_{j}}) \\= \ln \mylp \frac{S_{t_{j+1}}}{S_{t_{j}}} \myrp  \\= (\mu - \frac{\sigma^2}{2}) (t_{j+1} - t_{j}) + \sigma (W_{t_{j+1}} - W_{t_{j}})  \\= (\mu - \frac{\sigma^2}{2}) \delta t + \sigma \sqrt {\delta t} N(0,1) $
	\item  $S_{t_{j+1}} = S_{t_{j}} \exp \myblp (\mu - \frac{\sigma^2}{2}) \delta t + \sigma \sqrt {\delta t} N(0,1) \mybrp$
	\end{itemize}		
		
\end{itemize}

\item \hl{How to generate an rv N(0,1) from 12 rv $u_1,\ldots,u_12$ with $u_1$=Uniform $[0,1]$ ?}
\begin{itemize}
\item Answer: $X=\sum u_i - 6 \approx N(0,1)$
\item Recall Central limit $Z \sim \frac{\frac{1}{N} \sum_1^N X_i - \myE{X}}{\frac{\sigma(X)}{\sqrt{N}}}$
\item $E(U)=\int_0^1 u du = \frac{1}{2}$
\item $E(U^2)=\int_0^1 u^2 du = \frac{1}{3}$
\item $\sigma^2(U)=E(U^2)=E(U)^2=\frac{1}{3} - \frac{1}{4}=\frac{1}{12}$
\item $Z \sim \frac{\frac{1}{12} \sum_1^{12} U_i - \frac{1}{2}}{\frac{\sqrt{\frac{1}{12}}}{\sqrt{12}}}  = \sum U_i - 6 $
\item note: inefficient: requires 12 $u_i$ for just 1 N(0,1). Box-Muller uses 2 for 1.
\end{itemize}

\item \hl{Convergence rate of MC method ?}
\begin{itemize}
	\item $m$ time steps, $n$ simulations
	\item then convergence $\sim O \myblp \max(\frac{T}{m},\frac{1}{\sqrt{n}}) \mybrp $ \hlr{\kreuz}
\end{itemize}

\item \hl{What does reducing variance for MC ? how to do it ?}
\begin{itemize}
	\item it means: reduce the constant factor of the O(.)
	\item Methods: \hlr{\kreuz}
	\begin{itemize}
...
\item \hl{How to generate correlated N(.,.) variables with correlation $\rho$ ?}
\begin{itemize}
	\item given 2 uncorr $N_1,N_2$
	\item $X_1=N_1$
	\item $X_2=\rho N_1 + \sqrt{1-\rho^2}N_2$
	\item $corr(X_1,X_2)=corr(N_1,\rho N_1 + \sqrt{1-\rho^2}N_2) = corr(N_1,\rho N_1) + corr(N_1,\sqrt{1-\rho^2}N_2)=\rho corr(N_1,N_1) + \sqrt{1-\rho^2} corr(N_1,N_2)=\rho $
\end{itemize}

\item \hl{What is Newton's method? How does it converge ?}
\begin{itemize}
	\item Newton=recursive method for finding $x : f(x)=0$
	\item $x_{k+1}=x_k - \frac{f(x_k)}{f'(x_k)} \looparrowleft f(x_{k+1})-f(x_k)=(x_{k+1}-x_k) f'(x_k)$ with $f(x_{k+1})=0$
	\item Convergence rate : quadratic
	\item Proof
	\begin{itemize}
		\item let $f(x^*)=0$
		\item Taylor: $f(x^*)=f(x_k)+(x^*-x_k)f'(x_k)+\frac{1}{2}{(x^*-x_k)}^2 f''(c_k)$
		\item $x_{k+1}-x_k = - \frac{f(x_k)}{f'(x_k)}$
		\item $x_{k+1}-x^* \\= x_k -x^* - \frac{f(x_k)}{f'(x_k)} \\= x_k -x^* - \frac{f(x_k)-f(x^*)}{f'(x_k)} \\= \frac{f(x^*)-f(x_k)+f'(x_k)(x^*-x_k)}{f'(x_k)} $
		\item substitute Taylor etc
	\end{itemize}
\end{itemize}

\item \hl{Finite difference method for Trinomial Tree?}
\begin{itemize}
	\item Forward Euler == Trinomial Tree \hlr{\kreuz}
\end{itemize}
